...
create_enum = \
    re.compile(
        r'^\s*CREATE\s+TYPE\s+({0})\s+AS\s+ENUM'.format(RE_IDENT2), re.I)
# 1=type
create_type = \
    re.compile(r'\s*CREATE\s+TYPE\s+({0})\s+AS'.format(RE_IDENT2), re.I)

# 1=?, 2=column, 3=type
r_column = r'^\s*,?\s*(ADD\s+COLUMN\s+)?({0})\s+({1})'.format(
    RE_IDENT, RE_ALLT)
...
alter_column = re.compile(
    r',?\s*ALTER\s+COLUMN\s+({0})'.format(RE_IDENT), re.I)
    ...
    def parseCharSequence(obj, s):
        ...
        while d != -1:
            if d == 0:
                ...
                chars += \
                    ''.join(chr(x)
                            for x in range(ord(c[d - 1]), ord(c[d + 1]) + 1))
                c = c[d + 2:]
            d = c.find('-')
    ...
    def __init__(self, att, params=None):
        ...
        if self.network.find(':') != -1:
            ...
            try:
                ...
                assert False, "{0}: invalid ipv6 {1}".format(
                    self, self.network)
            self.net = ip.first
            ...
            try:
                ...
                assert False, "{0}: invalid ipv4 {1}".format(
                    self, self.network)
...
class Model(object):
    ...
    PARAMS = {'size': int, 'offset': int,
              'null': float, 'seed': str, 'type': str}
...
def findGenerator(obj, params):
    ...
    for k in sorted(params):
        if k in sorted(GENERATORS):
            assert not found, \
                "{0}: multiple generators '{1}' and '{2}'".format(
                    obj, found, k)
            found = k
    return found
...
class Table(Model):
    ...
    def addUnique(self, atts, type=None):
        if opts.debug:
            debug(3, "addUnique " + str(atts) + " type=" + type)
        assert len(atts) >= 1
        if len(atts) == 1:
    ...
    def getAttribute(self, name):
        assert name.lower() in self.atts, "{0} Attribute {1}".format(
            self, name)
...
def self_run(validate=None, seed=None, pipe=False, op=[]):
    ...
    if opts.debug:
        debug(1, "self-test cmd: {0}".format(cmd))
...
if opts.validate:
    if opts.validate == 'unit':
        ...
        if opts.self_test_hack:
            lines.append(
                "--df T=Borrow A=borrowed:end='2038-01-19 03:14:07'\n")
...
for line in lines:
    ...
    if d:
        if opts.debug:
            debug(2, "explicit TA directive")
        current_table = all_tables[d.group(2).lower()]
        current_attribute = None
    d = df_att.match(line)
    if d:
        if opts.debug:
            debug(2, "set attribute")
    ...
    if is_ct:
        ...
        if q:
            if opts.debug:
                debug(2, "add unique")
            current_table.addUnique(
                re.split(r'[\s,]+', q.group(3)), q.group(2))
        else:
            # UNIQUE()
